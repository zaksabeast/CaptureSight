...
DenMenuLayout::DenMenuLayout(std::vector<std::shared_ptr<csight::game::swsh::Den>> dens, std::string title) {
  m_dens = dens;
  m_title = title;
}
...
tsl::elm::Element *DenMenuLayout::createUI() {
  ...
  for (u32 i = 0; i < m_dens.size(); i++) {
    auto den = m_dens[i];
    auto pkm = den->getPKM();
    std::string shinyTypeText = den->getShinyType() == csight::enums::ShinyType::Square ? " ■ " : " ★ ";

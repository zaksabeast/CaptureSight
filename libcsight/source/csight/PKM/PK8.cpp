...
#include <csight/Game/SWSH/CalculateRaidSeed.hpp>
...
namespace csight::pkm {
  ...
  PK8::PK8(u8 *data) : PKM::PKM() {
    std::copy(data, data + PK8::StoredSize, m_data);
  ...
  void PK8::crypt(void) {
    ...
    for (u32 i = 8; i < PK8::StoredSize; i += 2) {
  ...
  void PK8::shuffleArray(u8 shuffleValue) {
    ...
    u8 originalData[PK8::PartySize];

    std::copy(m_data, m_data + PK8::PartySize, originalData);
  ...
  std::string PK8::getMintedNatureString() { return utils::getIndex(resources::NatureList, this->getMintedNature()); }
  ...
  enums::Gender PK8::getOTGender() {
    u8 genderByte = *(u8 *)(m_data + 0x125) >> 7;
    return genderByte < 2 ? (enums::Gender)genderByte : enums::Gender::Genderless;
  }

  std::string PK8::getOTGenderString() { return utils::getGenderString(this->getOTGender()); }

  u64 PK8::getRaidSeed() { return game::swsh::calculateRaidSeed(this->getEC(), this->getPID(), this->getIVs()); }

  std::future<u64> PK8::getRaidSeedAsync() {
    return game::swsh::calculateRaidSeedAsync(this->getEC(), this->getPID(), this->getIVs());
  }

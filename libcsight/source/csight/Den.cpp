...
namespace csight {
  ...
  std::shared_ptr<raid::RaidPokemon> Den::GetPKM() { return std::make_shared<raid::RaidPokemon>(this->GetSeed(), this->flawlessIVs, this->species); };
  ...
  raid::RaidTemplate Den::FindSpawn(std::vector<raid::RaidTemplate> templates) {
    ...
    return utils::getLast(templates);
  }
}  // namespace csight
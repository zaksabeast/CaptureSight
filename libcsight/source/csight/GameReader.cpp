...
namespace csight {
  GameReader::GameReader() : raid::RaidDetails::RaidDetails() { m_isDebugServiceRunning = utils::checkIfServiceIsRunning("dmnt:cht"); }

  Result GameReader::Attach() { return dmntchtGetCheatProcessMetadata(&m_metadata); }

  bool GameReader::GetIsPokemonRunning() { return (m_metadata.title_id == SWORD_TITLE_ID) || (m_metadata.title_id == SHIELD_TITLE_ID); }

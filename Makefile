...
ifeq ($(strip $(CONFIG_JSON)),)
	jsons := $(wildcard *.json)
	ifneq (,$(findstring $(OUTPUT).json,$(jsons)))
		export APP_JSON := $(TOPDIR)/$(OUTPUT).json
	else
...
ifeq ($(strip $(ICON)),)
	icons := $(wildcard *.jpg)
	ifneq (,$(findstring $(OUTPUT).jpg,$(icons)))
		export APP_ICON := $(TOPDIR)/$(OUTPUT).jpg
	else
...
# The rust lib can count as phony since we can always run it.
# Cargo will check if any work needs to be done.
.PHONY:	all $(RUST_LIB)
